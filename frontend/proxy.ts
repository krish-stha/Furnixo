...
export function proxy(req: NextRequest) {
  ...
  const token = req.cookies.get("furnixo_token")?.value
  const userRaw = req.cookies.get("furnixo_user")?.value
  ...
  try {
    ...
    res.cookies.delete("furnixo_user")
    res.cookies.delete("furnixo_token")
    return res
  }
