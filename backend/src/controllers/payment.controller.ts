...
import { sendPaymentReceiptEmail, sendRefundRequestedEmail } from "../services/mail.service";
...
const COMPANY_NAME = process.env.COMPANY_NAME || "FURNIXO";
...
if (order.paymentStatus === "paid") {
  return res.status(200).json({ success: true, data: { alreadyPaid: true } });
    ...
    const payload = {
      ...
      purchase_order_id,
      purchase_order_name,
      customer_info: { name: "Furnixo User", email: "test@example.com" },
    };
...
async requestRefund(req: AuthRequest, res: Response) {
  ...
  // Only paid orders
  if (String(order.paymentStatus).toLowerCase() !== "paid")
    throw new HttpError(400, "Refunds are only available for paid orders");
 
  const st = String(order.status || "").toLowerCase();
  const RETURN_WINDOW_DAYS = 14;
 
  if (st === "delivered") {
    // ✅ 14-day return window — use deliveredAt if present, else updatedAt
    const refDate = (order as any).deliveredAt || (order as any).updatedAt;
    if (refDate) {
    ...
    throw new HttpError(
      400,
      "Refunds can only be requested for pending, confirmed, or recently delivered orders"
    );
  }
 
  const orderTotalRs = Number(order.total || 0);
  if (amountRs > orderTotalRs)
    throw new HttpError(400, `Refund amount cannot exceed the order total (Rs. ${orderTotalRs})`);
 
  const amountPaisa = Math.round(amountRs * 100);
 
  // Prevent exceeding total across multiple requests
  ...
  const alreadyPaisa   = existing.reduce((sum, r) => sum + Number(r.amountPaisa || 0), 0);
  const remainingPaisa = Math.round(orderTotalRs * 100) - alreadyPaisa;
 
  if (remainingPaisa <= 0)
    throw new HttpError(400, "You have already requested a refund for the full order amount");
 
  if (amountPaisa > remainingPaisa) {
    const maxRs = Math.floor(remainingPaisa / 100);
    throw new HttpError(400, `Maximum refundable amount remaining: Rs. ${maxRs}`);
  }
  ...
  try {
    ...
    console.error("sendRefundRequestedEmail failed:", e);
  }
