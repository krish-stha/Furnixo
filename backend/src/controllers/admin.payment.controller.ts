...
import { sendRefundStatusEmail } from "../services/mail.service";
...
export class AdminPaymentController {
  ...
  async approveRefund(req: AuthRequest, res: Response) {
    ...
    // ── notify user ──────────────────────────────────────────────────────
    try {
      const user = rr.user as any;
      if (user?.email) {
        await sendRefundStatusEmail({
          to:        user.email,
          userName:  user.fullName,
          status:    "approved",
          amountRs:  Math.round(rr.amountPaisa / 100),
          orderId:   String((rr.order as any)?._id || rr.order),
          adminNote: rr.adminNote ?? undefined,
        });
      }
    } catch (e) {
      console.error("sendRefundStatusEmail (approve) failed:", e);
    }
  ...
  async rejectRefund(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      console.error("sendRefundStatusEmail (reject) failed:", e);
    }
  ...
  async markProcessed(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      console.error("sendRefundStatusEmail (processed) failed:", e);
    }
